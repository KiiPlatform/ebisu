...
#endif
void kiiPush_recvMsgTask(void* sdata)
{
    int remainingLen;
    int byteLen;
    int topicLen;
    ...
    //for(;;)
    {
        ...
        else
        {
		    if (mqtt_connected() != 0)
		    {
                M_KII_LOG(kii->kii_core.logger_cb("kii-error: mqtt disconnected\r\n"));
				kii->_mqtt_endpoint_ready = 0;
				return;
		    }

			if (mqtt_data_available() != 0)
			{
			    return;
			}
...
#if(KII_PUSH_PING_ENABLE)
void kiiPush_pingReqTask(void* sdata)
{
#define MQTT_PING_DELAY_PER_TIME 100 //ms
    ...
    kii->delay_ms_cb(MQTT_PING_DELAY_PER_TIME);
}
#endif
