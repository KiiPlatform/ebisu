...
void light_parseLedControl(char* bucketName, char* objectID)
{
	...
	{
		DEBUG("Retrieve object failed, objectID:\"%s\"\r\n", objectID);
		return;
	}
	...
	if(strcmp(type, STR_JSON_TYPE_COMMAND) == 0)
	{
		if(light_parseJson(jsonObject, &light) == 0)
		{
			...
			{
				// DEBUG("do action response success\r\n");
			}
		}
		else
		{
			DEBUG("Invalid light control command\r\n");
		}
	}
...
int light_onBoarding(void)
{
	...
	{
		DEBUG("Get thing token failed\r\n");
		return -1;
	}
	else
	{
		DEBUG("Get thing token success\r\n");
		return 0;
	}
}
