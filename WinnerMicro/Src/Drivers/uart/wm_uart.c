...
int tls_uart_set_flow_ctrl(struct tls_uart_port *port,
        ...
        unsigned int status)
{
	...
	if ((port->opts.flow_ctrl == TLS_UART_FLOW_CTRL_HARDWARE) && 
...
int tls_uart_port_init(int uart_no, tls_uart_options_t *opts)
{
	...
	bufrx = malloc(TLS_UART_RX_BUF_SIZE);
	if (!bufrx)
            return WM_FAILED;
	memset(bufrx, 0, TLS_UART_RX_BUF_SIZE);
 	...
 	port->recv.head = 0;
 	port->recv.tail = 0;
 	buftx = malloc(TLS_UART_TX_BUF_SIZE);
	if (!buftx)
	{
		free(bufrx);
		return WM_FAILED;
