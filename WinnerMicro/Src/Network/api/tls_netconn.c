...
static void net_skt_tcp_send(struct tls_netconn *conn)
{
	struct tcp_pcb *pcb = conn->pcb.tcp;
	u32 len;
	err_t err;
	...
	*/
	len = conn->current_msg->len;
	remainlen = len;	
	dataptr = (u8_t *)conn->current_msg->dataptr + conn->current_msg->write_offset;
//	TLS_DBGPRT_INFO("*******remainlen=%d, len=%d, dataptr=%s\n", remainlen, len, dataptr);
	do {
		do {
			err = tcp_write(pcb, dataptr, remainlen, TCP_WRITE_FLAG_COPY);
			if (err == ERR_MEM) {
				remainlen >>= 1;
			}
		} while ((err == ERR_MEM) && (remainlen > 8));
		//TLS_DBGPRT_INFO("remainlen:%d, err:%d\n", remainlen, err);

		if (err == ERR_OK){
			err = tcp_output(pcb);
			remainlen = len - remainlen;
			len = remainlen;
		}

		if (remainlen > 0){
			dataptr += len;
			TLS_DBGPRT_INFO("#####remainlen=%d, len=%d,\n", remainlen, len);
			tls_os_time_delay(2);
		}
		else
		{
...
static void net_do_write(void *ctx)
{
    ...
    if (conn->proto == TLS_NETCONN_TCP) {
        ...
        if (conn->state != NETCONN_STATE_CONNECTED) {
            ...
            LWIP_ASSERT("already writing or closing", conn->current_msg == NULL &&
                    conn->current_msg->write_offset == 0);
            LWIP_ASSERT("net_msg->len != 0", net_msg->len != 0);
            conn->current_msg = net_msg;
            conn->current_msg->write_offset = 0;
            //conn->write_offset = 0;
            net_skt_tcp_send(conn);
	...
	net_msg->dataptr = pdata;
        ...
        if (conn->proto == TLS_NETCONN_UDP) {
        	...
        	if (conn->client){
	            net_msg->conn = conn;
		        net_msg->err = ERR_VAL;/* for debug : catch not set err */
	            err = tcpip_callback_with_block(net_do_write, net_msg, 0);
        	}else{
        		dl_list_for_each(client_conn, &conn->list, struct tls_netconn, list){
				if (client_conn->used){
					net_msg->conn = client_conn;
		        	net_msg->err = ERR_VAL;/* for debug : catch not set err */
	            	err = tcpip_callback_with_block(net_do_write, net_msg, 1);
				}
        		}
