...
*/
    ...
    if (ps->ps_type > 1 || ps->wake_type > 1 ) {
    ...
    if (ps->ps_type == 0) {
        if (auto_mode_set || wif->priv->scanning) {
        ...
        tls_wl_if_standby(wif, ps->wake_type, ps->delay_time,
                ps->wake_time); 
    }
#endif
    return CMD_ERR_OK; 
        ...
        struct tls_cmd_connect_t *conn)
{
    ...
    if (wpa_s->wif->wlan_create && 
            (mode != CMD_MODE_INTERNAL)) {
        /* 返回连接信息 */
        memcpy(conn->bssid, wpa_s->bssid, ETH_ALEN);
        conn->type = wpa_s->ssid_conf->mode;
        conn->encrypt = hostif_cipher2host(wpa_s->ssid_conf->pairwise_cipher,
                wpa_s->ssid_conf->proto);
        conn->ssid_len = wpa_s->ssid_conf->ssid_len;
        memcpy(conn->ssid, wpa_s->ssid_conf->ssid, wpa_s->ssid_conf->ssid_len);
        conn->channel = hostif_frequency_to_channel(wpa_s->assoc_freq);
        if (conn->type == 2 || conn->type == 1)
            conn->rssi = 30;
        else {
            if (wpa_s->bss)
                conn->rssi =  0xFF - (u8)wpa_s->bss->level;
            else
                conn->rssi = 30;
        }
        conn->res = 1;
	...
	wpa_s->key_mgmt = wpa_s->ssid_conf->key_mgmt;
	if(wpa_s->ssid_conf->mode != WPAS_MODE_AP){
		wpa_s->ssid_conf->scan_req = 3; //AT command JOIN;
	}
    supplicant_event_reload_config(wpa_s, true); 
	return CMD_ERR_OK;
#endif
 ...
 int tls_cmd_create_net( void )
{
	...
	if(apinfo == NULL)
		return CMD_ERR_MEM;
	ipinfo = tls_mem_alloc(sizeof(struct tls_softap_info_t));
	if(ipinfo == NULL){
		tls_mem_free(apinfo);
		return CMD_ERR_MEM;
	}
	...
	tls_cmd_get_key(&apinfo->keyinfo);
...
{
    tls_param_get(TLS_PARAM_ID_PSM, (void *)enable, (bool)0);
...
{
    tls_param_set(TLS_PARAM_ID_ONESHOT_CFG, 
            (void *)&oneshotflag,
            (bool)update_flash);
    return 0;
}
...
{
	tls_param_get(TLS_PARAM_ID_ONESHOT_CFG, (void *)oneshotflag,0);
	return 0;
}
