        ...
        char *res_resp, u32 *res_len)
{
    ...
    u8 state;
	struct tls_hostif *hif = tls_get_hostif();
    if ((tok->arg_found == 5) && (tok->op == ATCMD_OP_EQ)) {
        do {
        ...
        char *res_resp, u32 *res_len)
{
    ...
    if (set_opt) {
        do {
            ...
            } else if(bssid.enable == 0){
            	memset(bssid.bssid, 0xff, ETH_ALEN);
				int i;
				for(i=0; i<6; i++)
					printf("bssid[%d]=%x\n", i, bssid.bssid[i]);
        ...
        char *res_resp, u32 *res_len)
{
    ...
    if ((tok->op == ATCMD_OP_EP) && (tok->arg_found > 0)) {
        set_opt = 1;
        update_flash = 1; 
    } else if ((tok->op == ATCMD_OP_EQ) && (tok->arg_found > 0)){
        set_opt = 1;
    } else if ((tok->op == ATCMD_OP_QU) && (tok->arg_found == 0)) {
        set_opt = 0;
    } else if ((tok->op == ATCMD_OP_NULL) && (tok->arg_found == 0)) {
        ...
        char *res_resp, u32 *res_len)
{
    ...
    if (set_opt) {
		dns_len = strlen(tok->arg[0]);
		if ((dns_len > 31)|| (dns_len == 0)){
			*res_len = atcmd_err_resp(res_resp, CMD_ERR_OPS);
			return 0;
		}
		memcpy(local_dnsname, tok->arg[0], dns_len);
	...
	char *res_resp, u32 *res_len)
{
	...
	{
		if (atcmd_filter_quotation(&tmpmac, tok->arg[0])){
			*res_len = atcmd_err_resp(res_resp, CMD_ERR_OPS);
		}

		mac = malloc(ETH_ALEN);		
		if (mac){
			if (strtohexarray(mac, ETH_ALEN, tmpmac)< 0){
				*res_len = atcmd_err_resp(res_resp, CMD_ERR_OPS);
			}else{
...
static int atcmd_rfr_proc( struct tls_atcmd_token_t *tok, char *res_resp, u32 *res_len)
{
	...
	*res_len = sprintf(res_resp, "+OK=%04x", databuf[0]);
	...
	while(--Num)
	{
		sprintf((char *)buff, ",%04x", *pdatabuf++);
		strcat(res_resp, (char *)buff);
		*res_len += 2;
	}
	return 0;
...
{ 
	...
	{// set tx gain
		if (strtohexarray(TxGain, 12, tok->arg[0]) < 0)
		{
...
static int atcmd_lppstr_proc(struct tls_atcmd_token_t *tok, char *res_resp, u32 *res_len)
{
	...
	ret = hexstr_to_uinit(tok->arg[0], &flag_start);
	if(ret)
	{
		TLS_DBGPRT_INFO("ret = 0x%x\r\n", ret);
		*res_len = atcmd_err_resp(res_resp, CMD_ERR_INV_PARAMS);
...
Step1:	   AT+WWPS=get_pin
		   Pin code will be responsed; User should input this Pin to AP; and device will use this Pin code for connection;
Step2: 	   AT+WWPS=start _pin
___________________________ 
2:
Step1: 	  AT+WWPS=!set_pin,xxxx
		  User can set an Pin code to device, which might be the one pasted on the device; User should input this Pin to AP 
Step2: 	  AT+WWPS=start _pin

___________________________	
3: 	
Step1: 	  AT+WWPS=start _pin
		  Pin code is the default value, which pasted on the device, and stored in system during manufacturing

For PBC:
Step1:	 AT+WWPS=start _pbc
