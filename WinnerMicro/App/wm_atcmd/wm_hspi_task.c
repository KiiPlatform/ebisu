        ...
        struct tls_hspi_rx_desc *rx_desc)
{
    ...
    if (dest_type == 1) {
        ...
        skt_info.socket = 0;
        buf = (char *)ext_hdr + sizeof(struct tls_hostif_socket_info);
    } else {
...
static s16 tls_hspi_rx_cmd_cb(char *buf)
{
	...
	if(hspi->rx_msg_queue)
		    OSQPost(hspi->rx_msg_queue, (void *)HSPI_RX_CMD_MSG);
	return WM_SUCCESS;
}
...
static s16 tls_hspi_rx_data_cb(char *buf)
{
	...
	if(hspi->rx_msg_queue)
	    OSQPost(hspi->rx_msg_queue, (void *)HSPI_RX_DATA_MSG); 	
	return WM_SUCCESS;
}
