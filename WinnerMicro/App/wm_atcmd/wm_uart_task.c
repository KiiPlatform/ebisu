...
#define      UART1_RX_TASK_STK_SIZE          256 
...
static s16 tls_uart0_task_rx_cb(char *buf, u16 len)
{
	...
	if(uart->rx_mailbox)
		OSMboxPost(uart->rx_mailbox, (void *)MBOX_MSG_UART_RX);
	return WM_SUCCESS;
}
...
static s16 tls_uart0_task_tx_cb(struct tls_uart_port *port)
{
	...
	if(uart->tx_mailbox)
	          OSMboxPost(uart->tx_mailbox, (void *)MBOX_MSG_UART_TX);
	...
	{ // check "at+" char
			...
			(recv->buf[(recv->tail + 2) & (TLS_UART_RX_BUF_SIZE - 1)] == '+')) 
		{
         ...
         //   TLS_DBGPRT_INFO("ptr_eol = 0x%x\n", ptr_eol);
            ...
            tls_hostif_cmd_handler( hostif_uart_type, buf, cmd_len+1);
            tls_mem_free(buf);
            atcmd_start = NULL; 
        }
		else 
...
RESENDBUF:
    ...
    if (def_socket) {
        ...
        tls_hostif_send_data(&skt_info, uart_net_send_data, buflen);
    }
...
static int uart_fwup_rsp(int status)
{
	char *cmd_rsp = NULL;
	u32 len;
	...
	if(tls_hostif_process_cmdrsp(HOSTIF_MODE_UART1_LS, cmd_rsp, len))
	{
...
//TLS_DBGPRT_INFO("tx_msg->u.msg_event.buflen=%d\r\n", tx_msg->u.msg_event.buflen);			
		if(0 != memcmp(at_resp_ok,tx_msg->u.msg_event.buf + tx_msg->offset,3) 
