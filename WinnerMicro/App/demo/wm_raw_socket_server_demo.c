...
OS_EVENT *demo_raw_sock_s_q = NULL;
...
err_t  raw_sk_server_recv(u8 skt_num, struct pbuf *p, err_t err)
{
	...
	do
	{
		...
		if(opentx)
			OSQPost(demo_raw_sock_s_q,(void *)DEMO_MSG_SOCKET_RECEIVE_DATA);
...
void  raw_sk_server_err(u8 skt_num, err_t err)
{
	...
	OSQPost(demo_raw_sock_s_q,(void *)DEMO_MSG_SOCKET_ERR);
}
...
{
	demo_raw_sock_s_q = OSQCreate((void **)&demo_raw_sock_s_queue, DEMO_QUEUE_SIZE);
...
static void raw_sock_s_net_status_changed_event(u8 status )
{
	switch(status)
	{
		case NETIF_WIFI_JOIN_FAILED:
			OSQPost(demo_raw_sock_s_q,(void *)DEMO_MSG_WJOIN_FAILD);
			break;
		case NETIF_WIFI_JOIN_SUCCESS:
			OSQPost(demo_raw_sock_s_q,(void *)DEMO_MSG_WJOIN_SUCCESS);
			break;
		case NETIF_IP_NET_UP:
			OSQPost(demo_raw_sock_s_q,(void *)DEMO_MSG_SOCKET_CREATE);
			break;
		default:
			break;
	}
...
//////	
	...
	for(;;) 
	{
    		msg = OSQPend(demo_raw_sock_s_q, 0, &error);
		//printf("\n msg =%d\n",msg);
		switch((u32)msg)
		{
