...
public class IAPService {
    ...
    public JSONObject startOrderTransaction(StartTransactionParam param) {
        ...
        if (product.getConsumeType() == ConsumeType.permanent
                && receiptDao.existProduct(product.getProductID(), new UserID(param.getUserID()))) {
            throw new ServiceException(IAPErrorCode.PRODUCT_BOUGHT_ALREADY);
        }

        String data = transactionDao.createNewOrder(product, param);
        if (data == null) {
            throw new ServiceException(IAPErrorCode.ERROR_CREATE_TRANSACTION);
        }
        ...
        try {
            ...
            switch (param.getPayType()) {
                case alipay:
                    ...
                    break;
                case mm:
                    JSONObject jsonMM = new JSONObject(data);
                    jsObj.put("errorCode", 0);
                    jsObj.put("mm", jsonMM);
                    break;
                default:
    ...
    public String queryTransaction(String transactionID) {
        Transaction transaction = transactionDao.getOrderByTransactionID(transactionID);
        return transaction.getJsonObject().toString();
    }
}
