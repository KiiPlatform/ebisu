...
public class Result {
    ...
    public  void parseResult(String publicKey) {
        try {
            ...
            isSignOk = rs.contentEquals("9000");
            checkSign(result, publicKey);
            Utils.log("Result", "isSignOk?" + isSignOk);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private  boolean checkSign(String result, String key) {
        ...
        try {
            ...
            if (signType.equalsIgnoreCase("RSA")) {
                Utils.log("Result", "before doCheck, signContent is " + signContent);
                Utils.log("Result", "before doCheck, sign is " + sign);
                retVal = Rsa.doCheck(signContent, sign, key);
            }
        } catch (Exception e) {
            e.printStackTrace();
