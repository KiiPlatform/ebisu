    ...
    const char* content_type)
{
    ...
    khc_slist* list = khc_slist_append(kii->_req_headers, kii->_rw_buff, header_len, &kii->_req_headers_mc);
    if (list == NULL) {
    ...
    size_t content_length)
{
    ...
    khc_slist* list = khc_slist_append(kii->_req_headers, cl_h, header_len, &kii->_req_headers_mc);
    if (list == NULL) {
        return KII_ERR_ALLOCATION;
    }
