        ...
        else
        {
            ...
            if (rcvdCounter > 0)
            {
                if ((g_kii_push.rcvdBuf[0]&0xf0) == 0x30)
                {
                    ...
                    if (byteLen > 0)
                    {
                        totalLen = remainingLen+byteLen+1; //fixed head byte1+remaining length bytes + remaining bytes
                    }
                    else
                    {
                    ...
                    {
                        rcvdCounter = kiiHal_socketRecv(g_kii_push.mqttSocket, g_kii_push.rcvdBuf+bytes, totalLen - bytes);
                        if (rcvdCounter > 0)
                        {
                            bytes += rcvdCounter;
                        }
