...
int light_updateStatus(light_struct light)
{
	...
	UART_PRINT("action response json:\r\n%s\r\n", jsonObject);
	if(kiiExt_extension(STR_EXTENSION_DO_ACTION_RESPONSE, jsonObject) < 0)
	{
		UART_PRINT("do action response failed\r\n");
		return -1;
	}
	else
	{
...
void light_parseLedControl(char* bucketName, char* objectID)
{
	...
	char bodyUrlFinal[LIGHT_FIRMWARE_UPGRADE_URL_SIZE + 1];
	...
	{
		UART_PRINT("Retrieve object failed, objectID:\"%s\"\r\n", objectID);
		return;
	}
	...
	if(strcmp(type, STR_JSON_TYPE_COMMAND) == 0)
	{
		if(light_parseJson(jsonObject, &light) == 0)
		{
			...
			{
				// UART_PRINT("do action response success\r\n");
			}
		}
		else
		{
			UART_PRINT("Invalid light control command\r\n");
		}
	}
	...
	else if(strcmp(type, STR_JSON_TYPE_UPDATEPWD) == 0)
	{
		...
		{
			UART_PRINT("get password failed\r\n");
		}
	}
	...
	else if(strcmp(type, STR_JSON_TYPE_FIRMWAREUPGRADE) == 0)
	{
		...
		{
			UART_PRINT("get version name failed\r\n");
			return;
		}
		...
		{
			UART_PRINT("get body url failed\r\n");
			return;
		}
		memset(bodyUrlFinal, 0, sizeof(bodyUrlFinal));
		...
		//		UART_PRINT("bodyUrl:%s\r\n", bodyUrl);
		UART_PRINT("bodyUrlFinal:%s\r\n", bodyUrlFinal);
		UART_PRINT("versionName:%s\r\n", versionName);
		lightIf_firmwareUpgrade(bodyUrlFinal, versionName);
	}
	else
	{
		UART_PRINT("invalid type:"
		           "%s"
		           "\r\n",
		           type);
	}
...
void light_callback(char* jsonBuf, int rcvdCounter)
{
	...
	// UART_PRINT("Push callback: jsonbuf:\r\n%s\r\n", jsonBuf);
	...
	{
		UART_PRINT("get objectID failed\r\n");
		return;
	}
	...
	{
		UART_PRINT("get bucketID failed\r\n");
		return;
	}

	// UART_PRINT("bucketID:%s\r\n", bucketName);
	...
	else
	{
		...
		{
			UART_PRINT("Invalid bucket name\r\n");
		}
	}
}
...
int light_onBoarding(void)
{
	...
	{
		UART_PRINT("Get thing token failed\r\n");
		return -1;
	}
	else
	{
		UART_PRINT("Get thing token success\r\n");
		return 0;
	}
}
...
int light_init(void)
{
	...
	{
		UART_PRINT("Get mac address failed\r\n");
		return -1;
	}
	UART_PRINT("vendorID:%s\r\n", mVendorID);

	memset(mModelName, 0, sizeof(mModelName));
	if(lightIf_getIModelName(mModelName) < 0)
	{
		UART_PRINT("Get model name failed\r\n");
		return -1;
	}
	UART_PRINT("Model name:%s\r\n", mModelName);
	...
	{
		UART_PRINT("Light onbording failed\r\n");
		return -1;
	}
	else
	{
