...
void tio_handler_set_cb_push(
    tio_handler_t* handler,
    TIO_CB_CUSTOM_PUSH cb_custom_push,
    void* userdata)
{
    handler->_cb_custom_push = cb_custom_push;
    handler->_cb_custom_push_data = userdata;
}
...
static void _cb_receive_push(char* payload, size_t payload_length, void* userdata) {
    ...
    if (handler->_cb_custom_push != NULL) {
        skip = handler->_cb_custom_push(payload, payload_length, handler->_cb_custom_push_data);
    }
    if (skip == KII_FALSE) {
